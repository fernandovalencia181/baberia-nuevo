...
async function mostrarHorariosDisponibles(fecha, barberoId, citaId, contenedor){
    try{
        ...
        generarRangoHorarios(8, 40, 13, 20, duracionMinutos, horariosOcupados, contenedor, fecha);
        generarRangoHorarios(15, 20, 19, 20, duracionMinutos, horariosOcupados, contenedor, fecha);
...
// Función para generar horarios entre dos rangos
function generarRangoHorarios(inicioHora, inicioMinuto, finHora, finMinuto, duracion, horariosOcupados, contenedor, fechaSeleccionada = null) {
    let minutosTotales = inicioHora * 60 + inicioMinuto;
    const minutosFin = finHora * 60 + finMinuto;
    ...
    while (minutosTotales <= minutosFin) {
        ...
        // ----------- comprobar si el horario ya pasó (solo si la fecha es hoy) -----------
        let horarioPasado = false;
        if (fechaSeleccionada) {
            if (fechaSeleccionada < hoyStr) {
                // Si la fecha es anterior a hoy → todos los horarios bloqueados
                horarioPasado = true;
            } else if (fechaSeleccionada === hoyStr) {
                // Si la fecha es hoy → solo bloquear los horarios ya pasados
        ...
        if (horariosOcupados.has(horario) || horarioPasado) {
            ...
            boton.addEventListener('click', function () {
                ...
                boton.classList.add('seleccionado');
                contenedor.dataset.horaSeleccionada = horario;
                if (typeof cita !== "undefined") cita.hora = horario; // opcional si usás cita global
            });
        }
