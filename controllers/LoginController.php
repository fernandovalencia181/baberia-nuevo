...
class LoginController {
    ...
    public static function googleLogin() {
        ...
        // Usar la variable de entorno GOOGLE_REDIRECT_URI
    ...
    public static function agregarPassword(Router $router) {
        ...
        // ✅ Si ya tiene teléfono y contraseña, redirigimos directamente
        if (!empty($usuario->telefono) && $usuario->tiene_password == 1) {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            // validar teléfono obligatorio solo si no lo tiene
            if (empty($usuario->telefono) && empty(trim($_POST["telefono"] ?? ""))) {
            ...
            if (empty($alertas)) {
                // solo actualizar teléfono si no lo tenía antes
